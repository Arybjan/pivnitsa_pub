...
tests/                    - автоматические тесты (58 тестов)
specification.json        - спецификация архитектуры, схемы БД и контекст проекта для ИИ (AI / LLM)
.env                      - переменные окружения
.env.example              - пример переменных окружения
requirements.txt          - зависимости
```

## 🤖 О файле specification.json

Файл `specification.json` служит единым источником контекста архитектуры для ИИ-ассистентов (LLM / AI Tools). В нем формализованы:
- Архитектура всей системы микросервисов Pivnitsa Pub и распределение ролей в команде.
- Полная схема БД (PostgreSQL) всех связанных сервисов (Auth, Booking, Events, Venue, Payment, Notifications).
- Контракты межсервисного взаимодействия и брокера сообщений.
Благодаря этому ИИ при разработке любого модуля точно знает контракты, типы данных и связи всей системы.
